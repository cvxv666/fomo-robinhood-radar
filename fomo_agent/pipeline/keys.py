...
class Keyring:
    ...
    def check(self, conn: sqlite3.Connection, key: str) -> tuple[bool, str | None]:
        ...
        if hit and now - hit[0] < self.ttl:
            return hit[1], hit[2]
        row = conn.execute("SELECT chat_id FROM api_keys WHERE key=? AND revoked_at IS NULL", (key,)).fetchone()
        ok = bool(row) and pro.entitled(conn, row["chat_id"])
